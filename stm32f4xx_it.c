...
<<<<<<< HEAD
//int8_t step[ 4 ][ 2 ] = { { 7, 0 }, { 0, 7 }, { -7, 0 }, { 0, -7 } };
uint8_t step[ 4 ][ 2 ] = { { 0, 7 }, { 7, 0 }, { 0, -7 }, { -7, 0 } };
...
>>>>>>> 8e4a8863ee1ffadc6b01b497caba959c267d693e
/* Private function prototypes -----------------------------------------------*/
extern USB_OTG_CORE_HANDLE           USB_OTG_dev;
static uint8_t *USBD_HID_GetPos (void);
extern uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev);
void HID_Release();
unsigned int Real_Sqrt(unsigned long int M);
...
=======
    ...
    if (Phase==0)
    {
        ...
        NextX=MyX-1;
        NextY=Real_Sqrt(R*R-NextX*NextX);
        if (MyX==-100)
        {
            Phase=1;
        }
    ...
    {
        NextX=MyX+1;
        NextY=-1*Real_Sqrt(R*R-NextX*NextX);
        if (MyX==100)
        {
            Phase=0;
        }
